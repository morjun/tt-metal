...
class CmeLogits:
    ...
    ``values``: [1,1,rows,N] TILE bf16 — the real dot products.
    ``ids``:    [1,1,rows,N] TILE uint32 — the vocab id each score belongs to
                (HF's ``selected_canonical``, flattened in ``top_k`` x ``P`` C order).

...
void read_kv_mask_chunks_n_tier(
    ...
    //   a later token).
    //
    //   `decode_start_pos_tokens` is the cur_pos value at the FIRST decode step.
    //   Positions < decode_start_pos were generated by prefill, NOT decode, and
    //   are NEVER written to L1 — so they must not be read from L1 either, even
    //   if they fall within the [cur_pos - T_total, cur_pos] ring window.
    //

...
*   **Code Reference**: `mlp.py` Line 132 passes `memory_config=ttnn.L1_WIDTH_SHARDED_MEMORY_CONFIG` to the `ttnn.linear` operation. This ensures the output of the matrix multiplication stays in L1.
...
## 3. Weight Reconstruction & Partitioning
...
*   **Partitioning**:
    *   **Column Parallel** (Gate/Up/QKV): Weights are `torch.chunk`ed along `dim=0` (rows of the weight matrix).
    *   **Row Parallel** (Down/Output): Weights are `torch.chunk`ed along `dim=1` (columns of weight matrix).
*   **Code Reference**:
    *   `attention.py` Line 220: `torch.chunk(state_dict[...], num_devices_per_group, dim=0)`

## Parallelism Architecture Summary

### Galaxy (TG, 32-Chip Mesh)
*   **Strategy**: Heavy use of **L1 Width Sharding** for activations to minimize GDDR6 access.
*   **Communication**: `reduce_scatter_minimal_async` keeps data sharded after reduction.

### LoudBox / T3K (8-Chip Ring)
*   **Strategy**: Optimizes for Ring topology.
*   **Optimization**: **Fused AllGather Matmul** overlaps the communication of gathering inputs with the computation of the matrix multiplication, hiding latency.
